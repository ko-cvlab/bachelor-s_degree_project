...
def add_margin(inside, margin):#問題なし
...
def evalpts_u(curve, div):#問題なし
    range_u = np.linspace(0, 1, div, endpoint=True)#False -> True
...
def clip_rect(img, center, rect_size, direction):
    ...
    rotated_img = img[start[1]:end[1], start[0]:end[0], :].copy()#40*40の正方形でトリミング
...
def separability_snake(img, init, rect_size, div=40, N_limit=20, dif_limit=0.001, dif_abs=False,increasing_limit=0.01
                       ,ctrlpts_increasing=True, c=0.95, ctrlpts_size=8,w=0.95, debug=True, max_ctrlpts_size=20):
...
#     init_2 = init#コピー作成
    ...
    for _ in range(N_limit):#Nlimitは20ではなく、3とかの方がよい？？？
        print(f"{_}回目")
        print("")
        
        evalpts = evalpts_u(init,div)#パラメトリック曲線
#         print(f"evalpts:{evalpts}")
#         print("")
              
        mean_separability, updated_evalpts = update_evalpts(evalpts,img,rect_size.astype(np.int64),div,w)#ここが肝！！！
#         print(f"updated_evalpts:{updated_evalpts}")
#         print("")
        ...
        if ctrlpts_increasing:#ctrlpts_increasingはFalseの方がよい？？？
...
#         newcp_2 = np.array(init_2.ctrlpts) - margin
#         init_2.ctrlpts = newcp.tolist()
#         cut = img.copy()
#         result_pts = np.array(init.evalpts).astype(np.int32)
#         drawn = cv2.polylines(cut, [result_pts], False, (255, 255, 255), thickness=1)
#         img_list.append(drawn)
    newcp = np.array(init.ctrlpts) - margin
    init.ctrlpts = newcp.tolist()
    return init
...
def draw_point_to_img(pts, img):
    img_c = img.copy()
    drawn = cv2.polylines(img_c, [pts], False, (255, 255, 255), thickness=1)
    return drawn
...
def draw_point_to_img_2(pts, img, center_eye_left_y_1, center_eye_left_x_1, y_before, x_before):
    ...
    drawn = cv2.polylines(img_c, [pts], False, (255, 255, 255), thickness=3)
    return drawn

def make_ellipse_points(center,radius,theta,psize=40):
    points = np.array([[radius[0]*math.cos(u), radius[1]*math.sin(u)] for u in np.linspace(0,2*math.pi,num=psize, endpoint=False)]).T
